...
**The quality gate (non-negotiable):** a change is done only when it passes, in this order, the PHP 7.4–8.5 syntax sweep, then `./bin/phpcs -q`, then `./bin/phpstan` (after `./bin/update-sequra`) — i.e. **`phpcs` → `phpstan` must both pass**, all via the Docker `bin/` wrappers. Don't silence PHPStan. Two ignore patterns are sanctioned: (1) Magento's auto-generated factories, and (2) calls through integration-core's `CheckoutAPI`/`AdminAPI` — these return an `Aspects` proxy whose fluent methods dispatch via `Aspects::__call(): mixed`, so PHPStan can't resolve the concrete controller/return types. The aspect case is handled centrally by `phpstan.neon` (`ignoreErrors: '#Call to an undefined method …Aspects::#'`, plus `reportUnmatchedIgnoredErrors: false`); prefer typing the result with `/** @var SomeResponse $response */` (as in `Model/ExpressCheckout/AvailabilityEvaluator.php`) over a per-line `@phpstan-ignore-next-line`. Nothing else is sanctioned. For tests, see `Test/README.md`.
